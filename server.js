...
// ─── bullets ─────────────────────────────────────────────────────────────────
function fireB(ownerId, x, y, tx, ty, isPlayer, dmg, count=1, spread=0) {
  const a = Math.atan2(ty - y, tx - x);
  for (let i = 0; i < count; i++) {
    const ang = a + (i - (count-1)/2) * spread;
    bullets.push({ id: nextId++, ownerId, x, y, vx: Math.cos(ang)*13, vy: Math.sin(ang)*13, isPlayer, dmg, r:4.5, life:75 });
  }
}

// ─── bot AI ──────────────────────────────────────────────────────────────────
function botAI(e, dt, now) {
  // find nearest threat / target (players + other bots)
  const allCells = [...players.values(), ...bots.filter(b => b !== e)];
  const er = r2m(e.mass);
  let fx = 0, fy = 0;

  // relation to nearest player
...
function tick() {
  ...
  for (let i = bullets.length-1; i >= 0; i--) {
    ...
    if (b.isPlayer) {
      ...
      for (const [pid, p] of players) {
        if (dst(b.x,b.y,p.x,p.y) < r2m(p.mass)+b.r) { p.hp -= b.dmg; hit=true; break; }
      }
    }
  ...
  for (const [id, p] of players) {
    ...
    for (let i = bots.length-1; i >= 0; i--) {
      ...
      if (dst(p.x,p.y,e.x,e.y) < pr+er-5) {
        if (e.mass > p.mass*1.1)      { p.hp -= 0.6*dt; p.mass = Math.max(10,p.mass-0.08*dt); }
    ...
    for (const [id2, p2] of players) {
      ...
      if (dst(p.x,p.y,p2.x,p2.y) < pr+pr2-5) {
        if (p.mass > p2.mass*1.15)       { p2.hp -= 0.8*dt; }
        else if (p2.mass > p.mass*1.15)  { p.hp  -= 0.8*dt; }
      }
    }
...
function respawnPlayer(p) {
  ...
  p.mass = 20; p.vx = 0; p.vy = 0; p.hp = 100;
...
function buildState() {
  return {
    players: [...players.values()].map(p=>({
      id:p.id, name:p.name, x:p.x, y:p.y, mass:p.mass, hp:p.hp,
    ...
    bullets: bullets.map(b=>({id:b.id,x:b.x,y:b.y,isPlayer:b.isPlayer,ownerId:b.ownerId})),
  };
}
...
const server = Bun.serve({
  ...
  websocket: {
    open(ws) {
      // player created after 'join' message
    ...
    message(ws, raw) {
      ...
      if (msg.type === 'join') {
        ...
        const player = {
          ...
          mass: 20, vx:0, vy:0, hp:100,
          targetX: WORLD/2, targetY: WORLD/2,
          hue, score:0, deaths:0,
          lastShot: 0,
          upgrades: [0,0,0,0,0], // speed, cadence, damage, double, minigun
        };
      ...
      if (msg.type === 'input') {
        ...
        if (msg.shoot) {
          ...
          if (now - p.lastShot >= cd) {
            ...
            const count = p.upgrades[4] ? 3 : p.upgrades[3] ? 2 : 1;
            const spread = (p.upgrades[3]||p.upgrades[4]) ? 0.13 : 0;
            fireB(p.id, p.x, p.y, msg.tx, msg.ty, true, dmg, count, spread);
          }
        }
      }
      ...
      if (msg.type === 'upgrade') {
        const p = players.get(ws._playerId);
        if (!p) return;
        const costs = [50,75,100,200,500];
        const maxes = [5,5,5,1,1];
        const i = msg.index;
        if (i<0||i>4) return;
...
function getCooldown(upg) {
  return upg[4] ? (100 - upg[1]*10) : (420 - upg[1]*60);
}
